...
def get_condense_grade(data):
    ...
    num_stu = len(stu)
    num_sem = len(sem_id)
    data = data.loc[data[' Student Identifier(ppsk)'].isin(stu), [' Student Identifier(ppsk)', 'Semester Year Name Concat', 'course', 'Grade Subtype Desc']]
    ...
    for key in data.groups.keys():
        ...
        for sem in stu_sem.groups.keys():
            ...
            for c in sem_grade.groups.keys():
                ...
                dic.append((course_id[c], grade))
            mat_data[ppsk_id[key]][sem_id[sem]] = dic
        print(mat_data[ppsk_id[key]])
    mat_file = {'stu_sem_grade_condense': mat_data}
    f = open('stu_sem_grade_condense.pkl', 'wb')
...
if __name__ == '__main__':
    ...
    get_condense_grade(enrollment_data)