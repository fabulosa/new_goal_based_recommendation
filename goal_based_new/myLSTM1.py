...
class long_term_LSTM(nn.Module):

    def __init__(self, ckt_model1, ckt_model2, dim_input_course, dim_input_grade, target_course):  # number of planning semesters == 3
        super(long_term_LSTM, self).__init__()

        self.target_course = target_course
        split = self.target_course.split(' ')
        self.t_num = int(''.join(list(filter(str.isdigit, split[0]))))
        self.t_sub = ' '.join(split[1:])
        self.dim_input_course = dim_input_course
        self.dim_input_grade = dim_input_grade

        self.past_model = ckt_model1  #'/research/jenny/RNN3/train/model1/nw_LSTM_cat_cat_1_50drp0wd1e-05clp0.pkl'
        ...
        self.target_prereqs_sub_filter = pickle.load(open('/research/jenny/RNN3/prerequisite_evaluation/target_sub_pre_sub_filter.pkl', 'rb'))
        subject = pickle.load(open('/research/jenny/RNN3/prerequisite_evaluation/subject_id.pkl', 'rb'))
        self.subject_id = subject['subject_id']
        self.id_subject = subject['id_subject']
        self.course_id_sub_id = pickle.load(open('/research/jenny/RNN3/prerequisite_evaluation/course_id_sub_id.pkl', 'rb'))
        f = open('/research/jenny/RNN/data_preprocess/course_id.pkl', 'rb')
        course = pickle.load(f)
        self.course_id = course['course_id']
        self.id_course = course['id_course']

        f = open('/research/jenny/RNN3/student_evaluation/sem_courses.pkl', 'rb')
    ...
    def cal_hiddenstates_output(self, padded_input, seq_len, courses_not_taken, k, model, hidden):  # hidden: initial hidden states
        ...
        rec_courses3 = np.where(y_B > 0.5)[0]
