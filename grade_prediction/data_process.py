...
def get_data_from_condense_seq(t):
    ...
    # data is a nested list, each row is a list: [[[course_id, grade_id]],[...],...[...]], each [] is for a semester.
    data = np.array(data)
    train = data[:, :t]
    vali = data[:, :t+1]
    f.close()
...
# padding
# input: each row: [[(course_id, grade_id), ...],[...],...[...]], each [] is for a semester. output: dim(batchsize, max_semester_number, dim_grade+dim_course)
def process_data(index, data, batchsize, dim_input_course, dim_input_grade):
    ...
    input_padded = np.zeros((batchsize, int(max_seq), dim_input_course + dim_input_grade), int)  # padded input
