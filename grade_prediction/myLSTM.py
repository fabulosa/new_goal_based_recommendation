...
class GradePrediction(nn.Module):
    ...
    def __init__(self, type, dim_input_course, dim_input_grade, nb_lstm_layers, nb_lstm_units, batch_size, drp_rate):
        super(GradePrediction, self).__init__()
        self.type = type
        self.dim_input = dim_input_course + dim_input_grade
        self.dim_output = dim_input_grade
        self.nb_lstm_layers = nb_lstm_layers
        self.nb_lstm_units = nb_lstm_units
        self.batch_size = batch_size
        if self.type == 'LSTM':
            self.lstm = nn.LSTM(input_size=self.dim_input, hidden_size=nb_lstm_units, num_layers=nb_lstm_layers, batch_first=True)
        elif self.type == 'RNN':
            self.lstm = nn.RNN(input_size=self.dim_input, hidden_size=nb_lstm_units, num_layers=nb_lstm_layers, batch_first=True)
        elif self.type == 'GRU':
            self.lstm = nn.GRU(input_size=self.dim_input, hidden_size=nb_lstm_units, num_layers=nb_lstm_layers, batch_first=True)
        ...
        if drp_rate > 0:
            ...
            self.drp = False
        self.hidden_to_output = nn.Linear(nb_lstm_units, self.dim_output)
    ...
    def forward(self, batch_data, batch_data_length):
        ...
        pack_input = torch.nn.utils.rnn.pack_padded_sequence(batch_data, batch_data_length, batch_first=True)
        ...
        linear_in = lstm_out.view(-1, lstm_out.shape[2])
