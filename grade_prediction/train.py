...
def train(model, optimizer, loader, train_data, epoch):
    ...
    for step, (batch_x, batch_y) in enumerate(loader):  # batch_x: index of batch data
        ...
        processed_data = process_data(batch_x.numpy(), train_data, batchsize, dim_input_course, dim_input_grade)
...
def train_and_evaluate(model, train_data, vali_data, optimizer, tolerance=5):
    ...
    while True:
        ...
        evaluation_loss = evaluate_loss(model, vali_loader, vali_data, batchsize, dim_input_course, dim_input_grade)
...
if __name__ == '__main__':
    ...
    model = LSTM.GradePrediction(model_type, dim_input_course, dim_input_grade, nb_lstm_layers, nb_lstm_units, batchsize, drop_out).cuda()
